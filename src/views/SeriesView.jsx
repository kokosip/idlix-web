...
import { getSeries, normalizeMediaItem } from '../services/api';

export default function SeriesView({ onSelectMedia }) {
    ...
    const fetchSeriesData = async () => {
      ...
      if (isMounted) {
        if (res.success && res.data) {
          const raw = Array.isArray(res.data) ? res.data : res.data.data || res.data.series || [];
          setSeriesList(raw.map((item) => normalizeMediaItem({ ...item, type: 'series' })));
        } else {
          setSeriesList([]);
        }
        setIsLoading(false);
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between border-b border-dark-border/60 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight">TV Series & K-Drama</h1>
            <p className="text-xs text-gray-400">Jelajahi serial TV dan drama Korea favorit (Halaman {page})</p>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="py-16 text-center text-gray-400 bg-dark-card/40 rounded-3xl border border-dashed border-dark-border">
          Tidak ada serial TV ditemukan pada halaman ini.
        </div>
      )}
